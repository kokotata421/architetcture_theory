...
本記事内でViewControllerの責務が理解しづらい原因としてコードレベルにおいてViewControllerが雑多な処理を抱えている状況を言及しましたが、実際にはそれ以外にも責務が環境によって変化してしまうViewControllerの性質が挙げられると思います。  
...
このようにViewControllerの責務の範囲はその環境で大きく変わります。  
そのため各コード例を参考にViewControllerの一般的な責務を理解するのは難しいと思います。  
### Appleなど信頼できるソースが例示するコードも設計的に必ず正しいわけではない
それは話を一歩広げると、設計をしっかりと理解するためにはコードではなく理論を学ぶ必要があるということです。(後ほど説明しますが、これはViewControllerに限った話ではありません。)      
プログラマが学習する際には他人のコードを見ることが多いと思います。  
ただ先ほどViewControllerを例に述べた通り、各コンポーネントの責務の境界はその状況によって大きく変化します。  
そのため設計を理解するのに他人のコードが参考になるとは限りません。  
...
またViewController以外の例では、Swift UIの公式チュートリアルではLandmarkというDomainオブジェクトではSwift UIとCoreLocationのフレームワークをインポートして利用しています。
```
import Foundation
import SwiftUI
import CoreLocation
...
ViewControllerは特に責務が定まっていないコンポーネントではありますが、このように私たちが普段目にするコードは特定の目的に沿って簡易的に実装されているものも多く設計論的観点はあまり考慮されていない場合があります。  
そのためコードを通して設計を学ぶのには限界があり(もちろん細かな記述やテクニックで参考にはなると思います)、しっかり理解を深めるためには理論を通して全ての状況に通底する原理・原則を学んでいく必要があるでしょう。  
