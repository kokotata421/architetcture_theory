...
##### プログラムサイズの統一化
そして画面仕様毎のプログラムへの影響が小さくなったことで統一化されるのはその構造だけではありません。  
ViewControllerのプログラムサイズも比較的統一されます。  
ViewController自身で具体的な処理を行っていた場合、そのプログラムサイズは処理内容によって大きく異なります。  
小さいものであれば100行程度、大きいものであれば400~600行、もしくはそれ以上ということもあるかもしれません。  
さらにUI開発をInterface Builder(StoryBoard/Xib)で行っているか、もしくはコードのみで行っているかでこの開きはさらに大きくなると思います。  
しかし責務をViewControllerの外部に委譲した場合はそうした処理やUI生成も全てViewController外部に書き出されるためViewControllerのプログラムのサイズとは関係なくなります。  
その結果ViewControllerのプログラムサイズは単純に処理するイベントの量によって決定されることになります。  
そして画面毎のイベントの量には一定の上限があります。  
その画面のイベントの量はその画面の機能の数とも言い換えることができると思いますが、一般的に一つの画面にあまりに多くの機能を搭載することはユーザーを混乱させる原因となってしまうためプログラム以前にプロダクト(UX)としてよろしくありません。  
そのためある画面のイベントの量が他の画面と比べて極端に少ないことはあっても、極端に多いことはまずあり得ないと思います。  
そのため各画面が処理するイベント量に上限があり、プログラムサイズにも上限がありません。  
そのプログラムサイズの上限は行数でいえば約300行当たりだと思います。  
