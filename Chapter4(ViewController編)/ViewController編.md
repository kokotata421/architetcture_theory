...
##### プログラムサイズの統一化
ViewController具体的な処理を外部に委譲するとそのプログラムサイズは処理するイベントの量によって決定されるようになります。    
そして画面毎のイベント量の上限はその画面仕様にかかわらず大体決まっており、そのプログラムサイズも行数にして300行程度が上限になると思います。  
その理由はプロダクト(UX)的な観点から特定の画面のイベント量のみ多くなることは考えにくいからです。  
画面のイベント量は画面の機能数とも言い換えることができると思いますが、一般的に一つの画面にあまりに多くの機能を搭載することはユーザーを混乱させてしまいます。  
そのためある画面のイベントの量が他の画面と比べて極端に少なく100行足らずのサイズになるというようなことはあっても、極端にイベント量が多いことはまずあり得ず大体のViewControllerプログラムは200~300行の範囲に収まるはずです。  
