...
ドキュメントに書かれている通りContainer ViewControllerの設計で何よりも重要なのは**自身のChild ViewControllerへの干渉を最低限とする**ことです。  
そしてそのためにはまずContainer ViewController内にある各Content ViewControllerの設計が大切になってきます。        
各Content ViewControllerの設計が独立した形でしっかりとなされていれば、Container ViewControllerがやるべきことはChild ViewController間の連携を取り持つくらいであり、その連携の具体的な方法に関して今回の設計論の観点から特に説明することはないと思います。コメ:もちろんContainer ViewControllerのChild ViewControllerは必ずしもContent ViewControllerであるわけではなく、Container ViewControllerがある場合もあります。しかし結局この場合もそのChild ViewControllerであるContainer ViewControllerの中のContent ViewControllerの設計がまず大切であることには変わりなく、またContainer ViewController-Child ViewController間の設計に関してはChild ViewControllerが

またConatainer ViewControllerで操作するChild ViewControllerの親Viewの管理は通常のViewの管理と同じで、こちらも特に説明することはありません。   
...
本記事ではViewController内の責務を細かく切り分けて外部に委譲する設計を提案していますが、規模の小さなアプリでは開発・運用コストを考えると責務を細かく分けずにViewControllerに直接実装した方が合理的である場合が多いでしょう。  
このようにViewControllerの責務の範囲はその環境で大きく変わるため、個々の実装例を参考にViewControllerの一般的な責務を理解するのは難しいと思います。  
### Appleなど信頼できるソースが例示するコードも設計的に必ず正しいわけではない
それは話を一歩広げると、設計をしっかりと理解するためにはコードだけではなくその理論を学ぶ必要があることを意味しています。(後ほど説明しますが、これはViewControllerに限った話ではありません。)      
プログラマが学習する際には他人のコードを見ることが多いと思います。  
ただ先ほどViewControllerを例に述べた通り、各コンポーネントの責務の境界はその状況によって大きく変化します。  
そのため設計を理解する際には必ずしも他人のコードが参考になるとは限りません。  

これはApple等信頼できるソースが例示しているコードにおいても同様です。  
例えばWWDCでAppleが示しているViewControllerのコードには状態管理を行なっているものがありますが、(記憶が曖昧で実際にどこのセッションでそのような例が示されていたかは忘れてしまいました。)これはセッションの主旨を考慮して便宜的にそのような形式になっているだけであり、ViewControllerで状態管理を行う設計が常に正しいというわけではありません。    
またViewController以外の例を出すと、Swift UIの公式チュートリアルではLandmarkというDomainオブジェクトがSwift UIとCoreLocationのフレームワークをインポートしてそのAPIを利用しています。    
```
import Foundation
import SwiftUI
import CoreLocation
...
そのためコードを通して設計を学ぶのには限界があり(もちろん細かな記述やテクニックで参考にはなると思います)、しっかり理解を深めるためには理論を通して様々な状況に通底する原理・原則を学んでいく必要があります。        


## 補論:View以外の責務の委譲
本記事ではViewControllerの多くの責務を外部に委譲すると述べましたが、その具体的な方法についてはView以外特別なテクニックを使っているわけでもないので言及はしませんでした。  
...
があります。
インスタンスに外部から作用する方法としてはinit(初期化)、プロパティ、メソッドしかないので特に驚くようなことではないと思います。  
### Constructor Injectionの優位性
ただここで強調したいのは1の方法が他の方法とは質的に異なるという点です。  
簡単にいうと一般的に1の初期化時のDIが他の方法に比べて優れています。  
