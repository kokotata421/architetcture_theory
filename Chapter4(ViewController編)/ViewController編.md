...
しかしそれによってViewControllerプログラムの煩雑さはなくなりコードも画一化されます。  
...
ここではいくつか例を紹介することでViewControllerのコードが画一化されることを確認します。  
##### 例1:Viewの操作
...
```
   self.alert.show(strategy: alertStrategy)
```  
上記2つの変更後のコードを見ても、この節の冒頭でも説明した通りそれまでViewControllerで実装していた様々な処理が責務を委譲することで吸収され画一化されたのがわかるのではないでしょうか。   
具体的な処理を外部のコンポーネントで行うことでほとんどのViewController側の処理は他コンポーネントの単一のメソッドの呼び出し、もしくはプロパティの操作のみとなります。    

そしてさらに一歩深く言及すると、次の節でも説明するように処理を他へ委譲することでViewController内で宣言的なプログラミングが可能になっています。  
それによってViewControllerのコードが全体的に自然言語(英語)に近い表現になるため、連携するコンポーネントは様々でもそのコードの形式自体は同質化され統一的になります。  
