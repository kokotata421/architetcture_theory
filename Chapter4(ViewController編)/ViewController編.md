...
```
   self.alert.show(strategy: alertStrategy)
```  
上記2つの変更後のコードを見ても、この節の冒頭でも説明した通りそれまでViewControllerで実装していた様々な処理が吸収され画一化されたのがわかるのではないでしょうか。   
具体的な処理を外部のコンポーネントに委譲することでほとんどのViewController側の処理は他コンポーネントの単一のメソッドの呼び出し、もしくはプロパティの操作のみとなりました。  

これをさらに一歩深く言及すると、次の節でも説明するように処理を他へ委譲することでViewController内で宣言的なプログラミングが可能になっています。  
