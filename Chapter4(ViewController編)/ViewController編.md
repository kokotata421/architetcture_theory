...
私はViewControllerで一般的に普及しているいくつの手法で開発すると以下の問題が起こると考えています。  
...
ViewControllerの責務は概念上は上記のように簡潔にまとめることが可能ですが、実際のコードレベルではViewの操作、アニメーション、遷移処理、アラート操作、CollectionViewのデリゲート・データソース、その他オブジェクトの連携等、実にさまざまな処理が書かれており、中には簡単な状態変数の管理もViewControllerが行っているケースも見かけます。  
