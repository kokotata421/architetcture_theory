...
そのためViewControllerの概念的な理解は開発する上で基本的なガイドラインを示してくれはするものの、実際にコードを読む・書くという作業においては各ViewController毎に考えなくてはいけない部分が大きいです。(例えば詳しくはUseCase編で触れますが、UseCaseは「単一の目的を持った処理をカプセル化したもの」であり、このUseCaseの一般的な理解を得ることによってUseCase内の主目的となるメソッドは一つであることがわかるので実際に作業する時にコードをどう読めば(書けば)良いかがかなり具体的になります。対して冒頭で挙げたViewControllerの概念では大きくViewControllerで何がなされ、何がなされていないのかはわかっても実際にコードをどう読めば(書けば)良いかはわかりません。)  
...
ViewController(UIKit)には宣言的な記述に思える箇所もありますが、全体的には
