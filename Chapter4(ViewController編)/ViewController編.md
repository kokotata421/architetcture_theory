...
ViewControllerのコアを「UI/システムから(への)イベントを処理する機構」と定義することでView、Alert、遷移等の具体的な操作が二次責務となりましたが、このアーキテクチャではこれらの二次責務をViewControllerから他のコンポーネントに委譲します。  
しかし留意したいのはこれらの責務の委譲は必須というわけではありません。  
