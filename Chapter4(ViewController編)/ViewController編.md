...
具体的な処理を外部に委譲した場合、それらコードは全てViewControllerの外に書き出されるためプログラムサイズはコンパクトになり、またそのサイズは単純に処理するイベントの量によって決定されるようになります。(ViewControllerの主要責務が「イベント処理」であり、具体的な処理を外部に委譲しているならばその処理するイベント量によってプログラム量が決まるしかありません。)  
そして画面仕様にかかわらずViewConrollerのサイズは大体300行当たりが上限になると思います。  
