...
このようにViewControllerの実装内容は状況に応じて変化するのでそこから一般的な責務を理解することは難しいのですが、それは話を一歩広げると、設計をしっかりと理解するためにはコードではなく理論を学ぶ必要があるということです。  
プログラマとして色々と学ぶ際には他人のコードを見ることが多いと思いますが、先ほどViewControllerを例に述べた通り、各コンポーネントの責務の境界はその状況によって大きく変化します。  
そのため設計を理解するのに他人のコードが参考になるとは限りません。      
これはApple等信頼できるソースが例示しているコードにおいても同様です。  
例えばWWDCでAppleが示しているViewControllerのコードには状態管理を行なっているものがありますが、(記憶が曖昧で実際にどこのセッションでそのような例が示されていたかは忘れてしまいました。)  
これはセッションの主旨を考慮して便宜的にそのような形式になっているだけであり、ViewControllerの設計においてそれが常に正しいというわけではありません。  
またViewControllerの話からは離れますが、Appleが提供しているSwift UIのチュートリアルではLandmarkというDomainオブジェクトではSwift UIとCoreLocationのフレームワークをインポートして利用しています。
```
import Foundation
import SwiftUI
import CoreLocation
...
しかしこれも先ほどの例と同様Appleが誤っているというわけではなく、あくまでここでの趣旨はSwift UIの実装方法にあるためそれ以外の設計については便宜的に済ましているだけだと思います。  

このようにコードはその状況に応じて大きくその内容を変えるため、Appleが例示しているものでも設計的に参考にできるのかは定かではありません。(もちろんコーディングや細かいテクニックで参考になることはあると思います。)  
そのため設計を学ぶ際には
