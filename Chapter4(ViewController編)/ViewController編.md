...
ドキュメントに書かれている通りContainer ViewControllerは**自身のChild ViewControllerへの干渉を最低限とするべき**であり、そのためにはまず各Content ViewControllerの設計が重要になってきます。      
各Content ViewControllerの設計が独立した形でしっかりとなされていれば、Container ViewControllerがやるべきことはChild ViewController間の連携を取り持つくらいであり、その連携の具体的な方法に関して今回の設計論の観点から特に説明することはないと思います。コメ:もちろんContainer ViewControllerのChild ViewControllerとしてContainer ViewControllerがある場合もあります。しかし結局この場合もそのChild ViewControllerであるContainer ViewControllerの中のContent ViewControllerの設計が大事であることには変わりありません。  
