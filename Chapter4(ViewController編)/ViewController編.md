...
各Content ViewControllerが独立した形でしっかりと設計がなされていれば、自身の親であるContainer ViewControllerからの干渉はChild ViewControllerの生成・破棄の管理、Child ViewControllerのRoot Viewの操作、Child ViewController間の連携くらいに絞られるはずです。   
上記の責務の説明はContainer ViewController固有のAPIと深く関わっており、設計を主旨としている本記事の範囲を超えています。    
そのため本記事ではContent ViewControllerの設計に限定して話を進めます。  
