...
しかしViewControllerの責務の境界はその周辺の環境によって変化しており、それも責務の理解を難しくさせていると思います。  
...
本記事ではViewController内の責務を細かく切り分けて外部に委譲する設計を提案していますが、規模の小さなアプリでは開発・運用コストを考えると責務を細かく分けずにViewControllerに直接実装した方が合理的であると思います。  
### Appleなど信頼できるソースが例示するコードも設計的に必ず正しいわけではない
このようにViewControllerの実装内容は状況に応じて変化するためその責務を正確に理解することは難しいのですが、そこから派生してもう一点述べたいのが、設計をしっかりと理解するためにはコードではなく理論を学ぶ必要があるということです。  
