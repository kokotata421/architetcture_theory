...
ViewControllerは特に責務が定まっていないコンポーネントではありますが、私たちが普段目にするコードは特定の目的に沿って取捨選択された結果であり設計論的観点はあまり考慮されていない場合もあります。  
そのためコードを通して設計の理解を深めることには限界があり(もちろん細かな記述やテクニックで参考にはなると思います)、
ではなく理論を通して全てのコードに通底する原理・原則を学んでいく必要があります。  
