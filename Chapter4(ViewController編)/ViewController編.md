...
ViewControllerはその画面開発の要でありその一番重要役割はその画面全体の機能(イベント)をいち早く開発者に伝えることにあります。  
そのためViewControllerのプログラムサイズをコンパクトにしてその機能(イベント)のみを宣言的プログラミングで記述する形式はその役割と合致しており合理的であると考えています。  
