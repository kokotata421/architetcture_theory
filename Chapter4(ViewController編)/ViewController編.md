...
恐らく設計理論の理解よりも先にコード(実務)を通してプログラミングを学んでいる多くの開発者からすると上記のように様々な処理が含まれているViewControllerは「Viewを管理する存在」ではなく、「画面開発における便利屋的な存在」に思えてしまうのではないでしょうか。  
...
「[2つのアプリを書き直して見えてきたSwiftUIのよさとつらみポイント](https://engineering.mercari.com/blog/entry/20201216-4043b38af1/)」という記事の冒頭に書かれているコード例が参考になると思います。(両者を比較する例を本記事内で紹介しようとすると状況設定やコードの説明等それなりの文量になってしまうので割愛します。)  
