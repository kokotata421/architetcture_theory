...
しかしそれによってViewControllerプログラムの煩雑さはなくなりコード全体が同質化します。  
...
ここではいくつか例を紹介することでViewControllerのコードが同質化することを確認します。  
##### 例1:Viewの操作
...
```
   self.alert.show(strategy: alertStrategy)
```  
上記2つの変更後のコードを見ても、この節の冒頭でも説明した通りそれまでViewControllerで実装していた様々な処理が責務を委譲することで吸収され画一的になったのがわかるのではないでしょうか。   
具体的な処理を外部のコンポーネントで行うことでほとんどのViewController側の処理は他コンポーネントの単一のメソッドの呼び出し、もしくはプロパティの操作のみとなります。    

そしてさらに一歩深く言及すると、次の節でも説明するように処理を他へ委譲することでViewController内のプログラムが宣言的になっています。    
それによりViewControllerのコードが全体的に自然言語(英語)に近い表現になるため、連携するコンポーネントは様々でもそのコードの形式自体は同質化され統一的になります。  
...
このようなViewControllerの宣言的プログラミングによる実装は、先程の責務の統一化と合わさりViewControllerプログラムの可読性を大きく改善させます。  
...
##### プログラムサイズの統一化
そして画面仕様のViewControllerプログラムへの影響が小さくなったことでそのプログラムサイズも比較的統一されます。  
ViewController自身で具体的な処理を行っていた場合、そのプログラムサイズは処理内容によって大きく異なります。  
小さいものであれば100行程度、大きいものであれば400~600行、もしくはそれ以上ということもあるかもしれません。  
さらにUI開発をInterface Builder(StoryBoard/Xib)で行っているか、もしくはコードのみで行っているかでこの開きはさらに大きくなります。    
しかし責務を外部に委譲した場合、そうした処理コードは全てViewControllerの外に書き出されるため処理内容の影響は受けなくなります。  
その結果ViewControllerのプログラムサイズは単純に処理するイベントの量によって決定されるようになります。(ViewControllerの主要責務が「イベント処理」であり、具体的な処理を外部に委譲しているならばその処理するイベント量によってプログラム量が決まるしかありません。)  
そしてこのようにイベント量によってプログラムサイズが決まる構造の場合、画面仕様にかかわらずViewConrollerのサイズは大体300行当たりが上限になると思います。  
その理由はイベント量が多すぎる画面仕様はそもそもプログラム以前にプロダクト(UX)として良くないからです。  
画面のイベントの量は画面の機能の数とも言い換えることができると思いますが、一般的に一つの画面にあまりに多くの機能を搭載することはユーザーを混乱させてしまいます。  
そのためある画面のイベントの量が他の画面と比べて極端に少ないことはあっても、極端に多いことはまずあり得ないと思って良いでしょう。  
そして私のサンプルプロジェクトではViewControllerのサイズはどんなに多くとも大体300行当たりとなりました。  
   
このように外部に処理を委譲してViewControllerをコンパクトにしてもコードの絶対量が減るわけではないから意味がないと思う人もいるかもしれませんが、私はそうは思いません。  
ViewControllerはその画面開発の要でありその一番重要役割はその処理方法の実装などではなく、その画面全体の機能(イベント)をいち早く開発者に理解してもらうことにあります。  
そのためViewControllerのプログラムサイズをコンパクトにしてその機能(イベント)のみを宣言的プログラミングで記述する形式はその役割と合致しており有効であると考えています。  
