...
## 補論:View以外の責務の委譲
本記事ではViewControllerの多くの責務を外部に委譲すると述べましたが、その具体的な方法についてはView以外特に特別なテクニックを駆使してるわけでもないので言及はしませんでした。  
しかし全く言及しないのもどうかと思うのでここではそれらについて簡単に触れます。  
まずRouterは細かい方法はそれぞれで異なる箇所もありますが、基本的にこの記事にあるようにRouterにViewControllerを渡してそちらで通常の遷移処理を行うだけです。
詳しくは画面遷移編で説明します。  
またAlertもRouterと同じです。  
Alertの表示はつまるところViewControllerのpresentメソッドで行うため、大きく言えば遷移の一種です。  
そのため機構としては別に扱っているもののViewControllerからの責務の委譲方法はRouterと同じです。  
