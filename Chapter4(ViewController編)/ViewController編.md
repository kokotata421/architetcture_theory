...
### 1.「Viewの管理(Control)」と一言でいえど、実際には多種多様なプログラムが書かれる
記事冒頭の「ViewControllerの基本」では、Appleのドキュメントを引用してViewControllerの主要な責務を4点紹介し、またそれらはさらに「Viewの管理」と要約できると説明しました。  
しかし概念上そのように簡潔まとめることはできても実際のコードレベルでは、Viewの操作、アニメーション、遷移処理、アラート操作、CollectionViewのデリゲート・データソース、その他オブジェクトの連携等、実にさまざまな処理が書かれており、中には簡単な状態変数の管理もViewControllerが行っているケースも見かけます。  
MVCやレイヤードアーキテクチャといった設計理論の基礎を理解できている開発者であれば、そのような多様なコードが書かれているViewControllerにおいても適宜適切に開発を行っていくことができると思います。
ただ設計理論をまだ押さえられていない開発者にとっては上記のようにViewControllerに書かれている多様なコード
