...
この記事ではそのViewControllerの設計について説明します。  
...
ただ設計というものは本質的にプラットフォームには依存しておらず、その素材にViewController、SwiftUIのどちらを利用しようが、しっかりと学んでいけばそこで得られたノウハウは他のプラットフォームでも活きるはずです。    
...
その理由は初期化時のDIは依存しているデータをそのインスタンス生成時に渡しているため、自身の挙動を自身の設計によって保証することができているからです。  
必要なコンポーネントを渡されない限り生成されない、そういう制約を構造として持つことで生成後には自身に期待される挙動を正しく振る舞えるかどうかは内部の。    
それに対して2と3の方法はインスタンス生成後にどのような方法かはわかりませんが、プロパティかメソッドを通して外部からコンポーネントを渡されることを前提としており
