...
##### プログラムサイズの画一化
ViewController具体的な処理を外部に委譲するとそのプログラムサイズは処理するイベントの量によって決定されるようになりますが、そうするとほとんどのViewControllerプログラムは200~300行の範囲に収まります。    
その理由はプロダクト(UX)的な観点から特定の画面のイベント量には上限があるからです。  
画面のイベント量は画面の機能数とも言い換えることができると思いますが、一般的に一つの画面にあまりに多くの機能を搭載することはユーザーを混乱させてしまいます。  
そのためある画面のイベントの量が他の画面と比べて極端に少なくなることはあっても、反対に極端にイベント量が多くなることはまずあり得ないと思って良いでしょう。  
