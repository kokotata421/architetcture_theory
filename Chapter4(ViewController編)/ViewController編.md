...
具体的な型があることで開発をする際に「どのようにコードを読めば(書けば)良いか」悩む機会は大きく減りますし、また一般的なアプリケーション開発では「AViewController」「BViewController」のように同様のコンポーネントを複数定義・実装するためそうした型を使い回すことできます。  

なのでViewControllerのように型が決まっていない場合、実際にそれぞれのViewControllerを見て個々に対応していく必要があり  
