...
この記事を読んでくれている人の中には「設計は興味あるけど、今更ViewController(UIKit)かあ...」と思っている方もいると思います。  
私もその意見に同意します。  
ただそれでもモチベーションを持って本記事を読んで欲しいため、最初に私なりのViewControllerの設計を学ぶ意義を述べていきます。        
...
今から始めるならば、SwiftUIを素材に使っていたのではないかと思います。    

### それでも今からViewControllerで設計を学ぶ意義
しかし、今から振り返ると今回の試みの中ViewControllerを採用したことはとても良かった、と考えています。  
その理由は、まず、設計をその原理まで理解しようとするならばViewControllerかSwift UIか、またiOSかAndroidか、といった開発におけるプラットフォームはあまり関係ないからです。  
もちろんViewControllerとSwift UIの記述の仕方で異なったり、AndroidではViewの実装でxmlを直接操作したりと細かいところで独自に学ばなければいけないことはあります。  
しかし設計は本質的にプラットフォームには依存しておらず、その素材にViewController、SwiftUIのどちらを使用しようが、そこで得られたノウハウは他のプラットフォームでも間違いなく活きてきます。      
またSwiftUIの現時点(2021年5月時点)での仕様を考えると、たとえSwiftUIが主流になろうとも当分の間はUIKitがiOS開発にとって完全に不必要になるわけではなさそうです。  
そのためUIKit/ViewControllerを素材に設計を学んだとしてもそれは必ずiOS開発に対する知見の礎になりますし、そしてなにより、SwiftUIよりもより原始的な技術であるUIKitを通して学ぶことで他の開発環境でも柔軟に対応できる応用力が身につくと思います。  
...
一般的に開発者アプリ内で独自に定義するViewControllerのほとんどはContent ViewControllerで、Container ViewControllerはあまり独自で定義されることはないと思います。[<sup>*2</sup>](#footnote2)  
ただ、私たちが開発の際よく使用するNavigation ControllerやTab Bar Controller等、デフォルトでUIKit上に定義されているUIViewControllerからの派生クラスはContainer ViewControllerに該当します。  
...
### Constructor Injectionの設計的優位性
ただここで強調したいのは1の方法が他の方法とは質的に異なるという点です。  
簡単にいうと一般的に1のConstructor Injectionは他の方法に比べて設計的に優れています。  
Constructor Injectionは依存しているデータをそのインスタンス生成時に渡しているため、DIの方法がConstructor Injectionのみに限定されている場合には、そのオブジェクトは必要なコンポーネントを渡されない限り生成されないという制約構造を持っていることになります。  
そのためインスタンスの動作時には必要なコンポーネントが全て備わっていることが保証されており、各挙動の原因はそのオブジェクト内部において見つけ出すことができます。  

それに対して2と3の方法はインスタンス生成後にプロパティかメソッドを通して外部からコンポーネントが渡されることを前提としており、インスタンスのコンポーネントが全て揃っているかは保証されていません。     
このようなオブジェクトの仕様だと自身の実装は変更していないにも関わらず、他のどこかしらのコードを変更したことでそのオブジェクトの挙動も変わってしまう恐れがあり、バグの温床となります。  
  
もちろん仕様によってプログラム起動中に依存しているデータを差し替える必要がある場合等には2と3の方法を取ることは致し方ないことです。  
しかしそうでない場合にはオブジェクトの挙動の原因範囲を自身の実装内に限定できる1の方法でDIを行うのが理想だと思います。  
また1の方法の利点としてオプショナル(?)・有値オプショナル(!)型を利用する必要がない点もあります。  
2と3の方法では生成後に値を代入するためオプショナル・有値オプショナル型を利用する必要がでてきます。    
...
このように相互に依存しているインスタンス関係では基本的にどちらか一方でプロパティかメソッドを通したDIを採用することでその依存関係を解決します。  
