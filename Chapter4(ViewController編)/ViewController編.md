...
具体的な処理を外部に委譲した場合、それらコードは全てViewControllerの外に書き出されるためプログラムサイズはコンパクトになり、またそのサイズは単純に処理するイベントの量によって決定されます。  
そしてその仕様にかかわらず画面毎のイベント量の上限は大体決まっています。  
...
##### ViewControllerの責務をイベント処理に集中させ、サイズをコンパクトにする意味
