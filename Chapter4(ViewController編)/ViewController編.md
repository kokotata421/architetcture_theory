...
##### プログラムサイズの統一化
そして画面仕様毎のViewControllerプログラムへの影響が小さくなったことでそのプログラムサイズも比較的統一されます。  
...
しかし責務を外部に委譲した場合、そうした処理コードは全てViewControllerの外に書き出されるため、その結果ViewControllerのプログラムサイズは単純に処理するイベントの量によって決定されることになります。  
