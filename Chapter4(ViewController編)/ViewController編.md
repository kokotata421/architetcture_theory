...
### 3.ViewControllerによってプログラムのサイズが大きく異なる(対応する画面の仕様に大きく影響を受ける)
ViewControllerは対応する画面の仕様に大きく影響を受けますが、このように各ViewControllerによってその実装が大きく異なっていることは、一般的なViewControllerの性質を理解しているだけでは実際の開発においては不十分であることを意味しています。  
色々誤解が生まれそうな表現なので詳細に説明すると、まず私は一般的なViewControllerの性質の理解が実際の開発で役に立たないと言っているわけではありません。  
本記事の冒頭でViewControllerの責務を説明しましたが、そのような基本的な責務の理解はViewControllerに書くべきこと、書くべきではないことをはっきりさせますし、またライフサイクルの理解はViewControllerのコードをどのように読んで(書いて)いけば良いか知る助けとなります。  
しかしViewControllerのこのような基本的な理解は、大まかなところでは開発においても助けになりますが、あまり具体的な助けにならないというのが私の印象です。  

例えば、詳しくはUseCase編で触れますが、UseCaseは「単一の目的を持った処理をカプセル化したもの」であり、このUseCaseの一般的な理解を得ることによってUseCase内の主目的となるメソッドは一つであることがわかるので実際に作業する時にコードをどう読めば(書けば)良いかがかなり具体的になります。対して冒頭で挙げたViewControllerの概念では大きくViewControllerで何がなされ、何がなされていないのかはわかっても実際にコードをどう読めば(書けば)良いかはわかりません。)  
