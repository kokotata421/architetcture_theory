...
本アプリではまさに、下記の該当コードが示しているようにAlertの入力処理が出力処理処理から切り離し可能であり、タップ時の処理登録をUIButtonと同じように実装できるようになっています。    
このAlertでは動物の切り替え確認のため「See Cats(Dogs)」と「Stay(そのまま)」の2つのボタンが提示していて、それぞれが以下のコードの`.change`と`.cancel`に該当しており、「See Cats(Dogs)」(.change)が押されるとpresenterの表示動物の切り替え処理の呼び出しを行い、「Stay」(.cancel)が押されると特に処理を行わない仕様となっています。  
ちなみに既に説明した通り、AlertClientの`func register(handler: @escaping(Action) -> Void) -> Key`は登録処理の解除を行うためのKeyを返り値としていますが、本アプリではViewControllerのライフサイクルの間に登録解除をする必要もないため返り値は捨てています。  
