...
もしここで上記の問題に対して愚直にリアクションして、Cellとその他のイベント処理の切り離しを諦めて混在させると、ViewControllerの入出力のデータフローを切り離す構造まで破壊してしまいます。    
これでは「木を見て森を見ず」な状態になってしまいます。  
    
    
やはりiOSアプリにおいてCellは特殊なコンポーネントであり、それはUIKitの設計を見ても明らかです。  
そのため多少不規則になったとしても例外的なUIとしてCellとそのイベント処理をDataSource内部に閉じ込めた方が、全体の設計は単純かつ明快になります。  

その上でRxCocoa以外でこの問題の対処として、Cellで自身の状態の変更を検知してイベント処理する方法があると思います。  
