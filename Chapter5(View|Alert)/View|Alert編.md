...
#### 2.アプリ機能との連携が見えづらい
1で見たようにデフォルトのAlertでは自身の表示に必要な文字列、スタイル、タップ時の処理を一つ一つ設定していきますが、このような設定の詳細情報をみても実装者以外の開発者はそのAlertが何をしているのかイマイチよくわかりません。  
...
しかしデフォルトのAlertの仕様はこうした個々のアプリ機能と完全に切り離されており、Alertの実装ではそこで表示する文言等詳細な情報だけを確認できます。    
...
そのため「ViewController」編でもその内部構造を「入力データフロー」と「出力データフロー」とに区別しています。    
しかし、デフォルトAlertではその表示(出力)箇所でタップ時の処理(入力)を定義しており、出力と入力のデータフローを切り離すことができていません。    
これは言ってみれば、異なるデータフローが入れ子構造になってしまっている状態です。  
通常であればViewからの入力は`func addTarget(_ target: Any?, action: Selector, for controlEvents: UIControl.Event)`等を利用してViewの出力とは切り離された形で行われます。  
