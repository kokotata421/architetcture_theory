...
何度も述べている通りDataSourceオブジェクトの構造は基本的にはとても単純ですが、その詳細の仕様については対応するCell毎に結構異なります。  
そのため一つのDataSourceオブジェクトでそれら全てのCellに対応しようとすると、それぞれの仕様がプログラムからわかりづらくなったりする等、逆に扱いづらくなってしまうと思います。  
...
Cellの出力イベントもその画面の一部なのではないでしょうか。  

結論を言うとCellの入出力イベントは他のViewのイベントと異なり特殊であるため、DataSource内部で行って問題ありません。  
理由はCellの性質を考えるとその入出力イベントの影響は該当のCellのUI状態に限定されているはずであり、画面全体の機能には関与していないからです。    
画面全体に影響を及ぼすようなイベントはCollectionViewの入力として画面本体に直接属するViewModel(Presenter)が処理します。  
そのため画面機能に直接関係しないCellのイベントをDataSource内部で処理するのは、ViewController上のコードが煩雑になることを回避する合理的な設計だと思います。  
また詳しい詳細は省きますが、技術的な構造としてもCellの入出力イベント処理はAlertと似てその表示処理(出力)の際に定義する必要があるため、もしCellの入出力イベントをViewControllerに明示した上で
さらにAlert同様その入力と出力のデータフローを切り離そうするならばDataSourceはかなり強引な設計になってしまいます。  
